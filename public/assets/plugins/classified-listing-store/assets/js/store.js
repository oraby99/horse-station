...
;(function ($) {
    ...
    $(function () {
                    ...
                    if ($.inArray(banner.type, allowed_image_types) !== -1) {
                        if (banner.size <= max_image_size) {
                            ...
                            $.ajax({
                                ...
                                type: 'POST',
                                beforeSend: function () {
                                    banner_wrap.find('.rtcl-media-action').append("<span class='rtcl-icon-spinner animate-spin'></span>");
                                },
                                success: function (response) {
                                    banner_wrap.find('.rtcl-icon-spinner').remove();
                                    if (!response.error) {
        ...
        $(".rtcl-store-logo-wrap .rtcl-media-action")
            .on('click', 'span.remove', function () {
                var self = $(this),
                    logo_wrap = self.parents(".rtcl-store-logo-wrap"),
                ...
                if (confirm(rtcl_store.confirm_text)) {
                    $.ajax({
                        ...
                        type: 'POST',
                        beforeSend: function () {
                            logo_wrap.find('.rtcl-media-action').append("<span class='rtcl-icon-spinner animate-spin'></span>");
                        },
                        success: function (response) {
                            logo_wrap.find('.rtcl-icon-spinner').remove();
                            if (!response.error) {
                                logo_wrap.find('.rtcl-store-logo').removeClass('has-logo');
                                logo_wrap.find('.rtcl-store-logo').addClass('no-logo');1
                                logo_holder.html("");
                            }
                        },
                        error: function (jqXhr, json, errorThrown) {
        ...
        $(".rtcl-store-logo-wrap .rtcl-media-action")
            .on('click', 'span.add', function () {
                ...
                logoFile.on('change', function () {
                    var fileItem = $(this),
                        logo_wrap = addBtn.parents(".rtcl-store-logo-wrap"),
                        logo_holder = $('.logo', logo_wrap),
                        form = new FormData(),
                        ...
                        if (logo.size <= max_image_size) {
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    logo_wrap.find('.rtcl-icon-spinner').remove();
                                    if (!response.error) {
                                        logo_wrap.find('.rtcl-store-logo').addClass('has-logo');
                                        logo_wrap.find('.rtcl-store-logo').removeClass('no-logo');
